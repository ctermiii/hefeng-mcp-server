...
if (apiUrlArg) {
    ...
    if (apiUrl) {
        console.log(`使用命令行参数中的API密钥: ${apiUrl}`);
        HEFENG_API_URL = apiUrl;
    }
}
    ...
    try {
        if (name === "get-weather") {
            ...
            if (days === 'now') {
                ...
                const weatherUrl = `${HEFENG_API_URL}/v7/weather/now?location=${location}`;
                ...
                const weatherUrl = `${HEFENG_API_URL}/v7/weather/${days}?location=${location}`;
